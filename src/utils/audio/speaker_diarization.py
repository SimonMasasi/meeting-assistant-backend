...
class SpeakerDiarizationService:
    ...
    def diarize_turns(self, audio_bytes: bytes) -> list[tuple[str, float, float]]:
        """Raw speaker turns as (provider_label, start_s, end_s). No DB access."""
        return [
            (t["speaker"], t["start"], t["end"])
            for t in self._get_diarizer().diarize(audio_bytes)
        ]

    def diarize(
        self, audio_bytes: bytes, meeting_id: int, current_user_id: int
        ...
        turns = self.diarize_turns(audio_bytes)
